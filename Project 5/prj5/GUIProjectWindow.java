...
public class GUIProjectWindow {
    ...
    public GUIProjectWindow(LinkedList<GUIGlyph> GUIGlyphList) {
        ...
        sortGUIGlyphGenre(gList);
        sortHobby(gList);
        sortGUIGlyphTitle(gList);
        sortHobby(gList);
    ...
    public void sortHobby(LinkedList<GUIGlyph> gList) {
        ...
        for (int i = 0; i < output.length; i++) {
            ...
            System.out.println("Song Year: " + ((GUIGlyph)output[i]).getSong()
                .getDate());
            int[][] responses = ((GUIGlyph)output[i]).sortByHobby();
    ...
    public void sortMajor(LinkedList<GUIGlyph> gList) {
        ...
        for (int i = 0; i < output.length; i++) {
            ...
            System.out.println("Song Year: " + ((GUIGlyph)output[i]).getSong()
                .getDate());
            int[][] responses = ((GUIGlyph)output[i]).sortByMajor();
            for (int x = 0; x < responses.length; x++) {
                for (int y = 0; y < responses[0].length; y += 4) {
                    if (x == 0) {
                        System.out.println("Heard");
    ...
    public void sortRegion(LinkedList<GUIGlyph> gList) {
        ...
        for (int i = 0; i < output.length; i++) {
            ...
            System.out.println("Song Year: " + ((GUIGlyph)output[i]).getSong()
                .getDate());
            int[][] responses = ((GUIGlyph)output[i]).sortByRegion();
            for (int x = 0; x < responses.length; x++) {
                for (int y = 0; y < responses[0].length; y += 4) {
                    if (x == 0) {
                        System.out.println("Heard");
     ...
     */
    public void sortGUIGlyphGenre(LinkedList<GUIGlyph> GUIGlyphList) {
        ...
        for (i = arr.length - 1; i > 0; i--) {
            ...
            temp = ((GUIGlyph)arr[first]); // swap smallest found with element in
                                        // position
            // i.
            arr[first] = arr[i];
            arr[i] = temp;
     ...
     *            is the list of GUIGlyphs to be sorted
     */
    public void sortGUIGlyphTitle(LinkedList<GUIGlyph> GUIGlyphList) {
        Object[] arr = GUIGlyphList.toArray();
        int i, j, first;
        GUIGlyph temp;
        for (i = arr.length - 1; i > 0; i--) {
